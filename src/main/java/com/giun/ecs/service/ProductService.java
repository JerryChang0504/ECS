...
public class ProductService {
	...
	public Outbound getProductById(Integer id) {
		...
		ProductResp response = ProductResp.builder().id(product.getId())
				.name(product.getName()).price(product.getPrice())
				.stock(product.getStock()).description(product.getDescription())
		...
		List<ProductShow> result = productRepository.findAll().stream()
				.map(product -> {
					...
					return ProductShow.builder().id(product.getId())
							...
							.category(product.getCategory()).rating(null) // TODO:
																			// 根據實際資料庫欄位填入
																			// product.getRating()
							.imageBase64(
	...
	public Outbound updateProduct(Integer id, ProductUploadReq req) {
		...
		Product updateProduct = Product.builder().id(product.getId())
	...
	public Outbound productList() {
		...
		List<ProductResp> result = productRepository.findAll().stream()
				.map(product -> {
					...
					return ProductResp.builder().id(product.getId())
							.name(product.getName()).price(product.getPrice())
